...
if(isset($_POST['sbm'])){
    ...
    if($num_rows == 1){
        $error = '<div class="alert alert-danger">email đã trùng !</div>';
        //$sql = "INSERT INTO user (user_full, user_mail, user_pass) VALUE('$user_full', '$user_mail', '$user_pass')";
        // header('location: index.php?page_layout=user');	
    }else if($user_pass != $user_re_pass){
        $error = '<div class="alert alert-danger">mật khẩu ko trùng !</div>';
    }else {
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="user_level" class="form-control">
                                        <option <?php if(['user_level'] == 1){ echo 'selected'; } ?> value=1>Admin</option>
                                        <option <?php if(['user_level'] == 2){ echo 'selected'; } ?> value=2>Member</option>
                                    </select>
                                </div>
